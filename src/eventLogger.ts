...
export class EventLogger {
  ...
  getSessionInfo(): { 
    ...
    const now = Date.now();
    const remainingTime = this.sessionStartTime ? 
      Math.max(0, this.SESSION_TIMEOUT_MS - (now - this.sessionStartTime)) : null;
    
    return {
      sessionId: this.sessionId,
      isActive: !!this.currentLogFile,
  ...
  getEventsForSession(sessionId: string): GameEvent[] | null {
    if (sessionId === this.sessionId && this.currentLogFile) {
      return [...this.events];
    }
    return null;
  }
  ...
  private exportEventsToMarkers(events: GameEvent[], fps: number = 30): DaVinciResolveMarker[] {
    ...
    events.forEach((event, index) => {
      const frameNumber = Math.floor((event.timestamp - events[0].timestamp) / 1000 * fps);
